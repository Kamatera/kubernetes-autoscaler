...
def write_k8s_vars(name_prefix, kamatera_api_client_id, kamatera_api_secret, ssh_pubkeys=None):
    ...
    if not cluster_autoscaler_image:
        cluster_autoscaler_image = f'ghcr.io/kamatera/kubernetes-autoscaler:v{k8s_version}'
...
def reconstruct(kamatera_api_client_id, kamatera_api_secret, name_prefix):
    bastion_ssh_port = os.environ["KTBCA_BASTION_SSH_PORT"]
    nodes_ssh_port = os.environ["KTBCA_NODES_SSH_PORT"]
    keypath = os.environ["KTBCA_SSH_KEY"]
    tfdir = str(os.path.join(os.path.dirname(__file__), "..", "..", ".data", "cluster_autoscaler", name_prefix, "terraform"))
    if not os.path.exists(tfdir):
        os.makedirs(tfdir, exist_ok=True)
    bastion_public_ip = None
    for network in json.loads(subprocess.check_output([
        "cloudcli",
        "--api-clientid", kamatera_api_client_id,
        "--api-secret", kamatera_api_secret,
        "server", "info", "--name", f'{name_prefix}-bastion', '--format=json'
    ]))[0]["networks"]:
...
'''
    for server in json.loads(subprocess.check_output([
        "cloudcli",
        "--api-clientid", kamatera_api_client_id,
        "--api-secret", kamatera_api_secret,
        "server", "info", "--name", f'{name_prefix}-.*', '--format=json'
    ])):
        if server['name'].endswith('-bastion'):
            continue
        for network in server["networks"]:
                if network["network"].startswith('lan-'):
                    ssh_config += f'''
Host {server["name"]}
  HostName {network['ips'][0]}
  User root
  Port {nodes_ssh_port}
...
'''
                    break
    with open(os.path.join(tfdir, "ssh_config"), "w") as f:
        f.write(ssh_config)
    print(dedent(f'''
        export SSH_CONFIG={os.path.join(tfdir,  "ssh_config")}
        ssh -F $SSH_CONFIG {name_prefix}-controlplane1
        kubectl get nodes
    '''))
...
def main(action='setup'):
    ...
    if action == 'setup':
        ...
        if action == 'destroy':
            ...
            reconstruct(kamatera_api_client_id, kamatera_api_secret, name_prefix)
